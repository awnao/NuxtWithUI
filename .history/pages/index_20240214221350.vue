  ...
  <div class="info-container">
    <section class="info-section">
      ...
      <div class="p_v1">
        ...
        <UModal v-model="isOpen">
          <div style="text-align: center" class="p-4 row flex justify-center">
            <ul>
              <li v-for="(player, index) in players" :key="index">
                <p style="color: white">{{ player.name }} - Score: {{ player.score }}</p>
              </li>
            </ul>
          </div>
        </UModal>
    ...
    <section class="info-section">
      ...
      <iframe
        width="560"
        height="315"
        src="https://www.youtube.com/embed/HYqnkQDIGjo"
...
export default defineComponent({
  data() {
    return {
      ...
      players: [] as { name: string; score: number }[],
    };
  },
  async mounted() {
    try {
      ...
      this.players = data["38.7.218.60:27021"].players.map((player: any) => ({
        name: player.name,
        score: player.score,
      }));
    } catch (error) {
  ...
  setup() {
    ...
    const modoInfo = "REGLAMENTO GENERAL:
    Cualquier tipo de insulto hacia un admin será motivo de BAN
	  Prohibido abusar del micrófono, en caso de hacerlo le corresponderá GAG
	REGLAMENTO EN CERRADOS:
	El FRAG estará permitido mientras pueda estando el antifrag activo, salvo excepciones que serán determinadas por el admin.^n")
	En el area chica solo podrá ingresar un player que será el arquero del equipo.^n")
	El arquero no podrá sobrepasar la mitad de la cancha.^n")
	No se podrá cambiar el arquero, salvo excepcion del adm.</UL>^n^n") "
